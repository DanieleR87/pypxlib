...
databases. The version of pxlib currently exposed by pypxlib is 0.6.10.
...
and ``libpx.dylib`` were obtained from building pxlib 0.6.10 on Ubuntu
...
This project contains dynamic libraries for version 0.6.10 of the pxlib
...
.. code:: bash
    ...
    wget 'http://downloads.sourceforge.net/project/pxlib/pxlib/0.6.10/pxlib-0.6.10.tar.gz?ts='`date +%s`'&use_mirror=freefr' -O pxlib-0.6.10.tar.gz
    tar -zxvf pxlib-0.6.10.tar.gz
    cd pxlib-0.6.10/
    ./configure
    make
    sudo make install
...
.. code:: bash
    ...
    curl -L 'http://downloads.sourceforge.net/project/pxlib/pxlib/0.6.10/pxlib-0.6.10.tar.gz?ts='`date +%s`'&use_mirror=freefr' -o pxlib-0.6.10.tar.gz
    tar -zxvf pxlib-0.6.10.tar.gz
    cd pxlib-0.6.10/
...
3. Download the pxlib 0.6.10 sources from
...
4. Extract the pxlib sources to *two* directories for 32 and 64 bit,
   respectively. Eg. ``C:\pxlib-0.6.10-x86`` and ``C:\pxlib-0.6.10-x64``.
5. Start the *Visual C++ 2008 32-bit Command Prompt*, cd to
   ``C:\pxlib-0.6.10-x86`` and execute the following commands:
.. code:: bash

    cmake -D CMAKE_CXX_FLAGS_RELEASE=/MT -DCMAKE_BUILD_TYPE=Release -D PX_HAVE_ICONV=0 -D PX_HAVE_RECODE=0 .
    nmake

6. Repeat step 5. with the *64*-bit Command Prompt and ``C:\pxlib-0.6.10-x64``.
7. That's it. You now have the 32 bit dll in ``C:\pxlib-0.6.10-x86\pxlib.dll``
   and the 64 bit dll in ``C:\pxlib-0.6.10-x64\pxlib.dll``.
